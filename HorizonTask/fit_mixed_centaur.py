...
import argparse
import torch
from models import MixedEffectBinomialRegression
...
inputs, num_B_choices, ids = torch.load('data/model=' + args.model + '_full_data.pth')
...
for fold in range(num_inner_folds):
    ...
    ids_train = ids[inner_training_index]
    ...
    ids_validation = ids[inner_validation_index]
    ...
    for alpha_index, alpha in enumerate(alphas):
        # fit model
        logreg = MixedEffectBinomialRegression(inputs_train.shape[1], alpha=alpha, num_groups=ids.max()+1).to(device)
        logreg.fit(
            inputs_train,
            num_choices_train,
            num_B_choices_train,
        ...
        llf_train[alpha_index, fold] = -Binomial(total_count=num_choices_train, logits=logreg(inputs_train, ids_train)).log_prob(num_B_choices_train).mean().item()
        llf_validation[alpha_index, fold] = -Binomial(total_count=num_choices_validation, logits=logreg(inputs_validation, ids_validation)).log_prob(num_B_choices_validation).mean().item()
...
logreg = MixedEffectBinomialRegression(inputs_train.shape[1], alpha=best_alpha, num_groups=ids.max()+1).to(device)
logreg.fit(
    inputs_train,
    num_choices_train,
    num_B_choices_train,
...
logits = logreg(inputs_test, ids_test)
llf_test = -Binomial(total_count=num_choices_test, logits=logits).log_prob(num_B_choices_test).sum().item()

print(llf_test)
torch.save([llf_test, best_alpha, logits], 'data/loo_centaur_mixed_model=' + args.model + '_fold=' + str(args.foldid) + '.pth')
